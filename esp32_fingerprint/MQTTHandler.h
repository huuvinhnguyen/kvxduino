...
class MQTTHandler {
  ...
  private:
    ...
    String generateRandomUUID() {
      ...
      uint32_t part1 = esp_random();  // First 32 bits
      uint32_t part2 = esp_random();  // Second 32 bits
      uint32_t part3 = esp_random();  // Third 32 bits
      uint32_t part4 = esp_random();  // Fourth 32 bits
      ...
      snprintf(uuid, sizeof(uuid), "%08X-%04X-%04X-%04X-%012X",
               ...
               (uint16_t)(part2 >> 16), (uint16_t)(part2), // Second 4 and Third 4 characters
  ...
  public:
    ...
    void connectMQTT() {
      ...
      if (client.connected()) {
        Serial.println("connected_connectMQTT");
        return;
      }
...
//      String clientId = String(chipId) + String(WiFi.macAddress()) + "1";
      ...
      if (client.connect(clientId.c_str())) {
        ...
        String fingerprint = deviceId + "/fingerprint";
        client.subscribe(fingerprint.c_str(), 1);
